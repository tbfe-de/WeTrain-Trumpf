...
struct SumFunctor {
    int &sum;
    SumFunctor(int &sum_) : sum(sum_) {}
    void operator()(int e) { sum += e; }
};
...
//  test_object.for_each([&sum](int e) { sum += e; });
    test_object.for_each(SumFunctor(sum));
//  --------------------------------------------------
...
#ifdef FULLY_GENERIC_FOR_EACH
//  ---------------------------------------------------
//  SumFunctor f;
//  test_object.for_each(f); // <-- wrong result as functors are copied
//  auto sum2 = f.sum;
    auto sum2 = test_object.for_each(SumFunctor()).sum;
//  ---------------------------------------------------
    BOOST_CHECK_EQUAL(sum2, 6);
#else
    SumFunctor f;
//  ----------------------------------
//  test_object.for_each(f); // <-- wrong result as functors are copied
    test_object.for_each(std::ref(f));
//  ----------------------------------
    BOOST_CHECK_EQUAL(f.sum, 6);
#endif
}
#endif
