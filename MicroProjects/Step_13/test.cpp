...
BOOST_AUTO_TEST_CASE(catchable_throw_in_dtor) {
    try {
//  	ThrowInDestructor> test_object;
    	BlockOf<ThrowInDestructor> test_object(2);
    }
    catch (int) { BOOST_CHECK(true); }
    catch (...) { BOOST_CHECK(false); }
}

BOOST_AUTO_TEST_CASE(uncatchable_throw_in_dtor) {
    try {
//  	ThrowInDestructor> test_object;
    	BlockOf<ThrowInDestructor> test_object(2);
        throw 0;
