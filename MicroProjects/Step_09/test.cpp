#define NEEDS_CONST_ITERATOR_TOO
#include "BlockOf.h"
...
BOOST_AUTO_TEST_CASE(range_for_loop) {
    BlockOf<int> test_object({1, 2, 3});
    BOOST_CHECK_EQUAL(test_object.size(), 3);
    int sum = 0;
    for (int v : test_object)
        sum += v;
    BOOST_CHECK_EQUAL(sum, 6);
}

#ifdef NEEDS_CONST_ITERATOR_TOO
BOOST_AUTO_TEST_CASE(range_for_loop_const) {
    const BlockOf<int> test_object({1, 2, 3});
    BOOST_CHECK_EQUAL(test_object.size(), 3);
    int sum = 0;
    for (int v : test_object)
        sum += v;
